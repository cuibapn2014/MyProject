...
@section('title', 'Thêm mới | Xuất kho')
@section('main')
@php
$current = 14;
@endphp
<div class="container px-6 mx-auto grid">
    ...
    @endif
    <form action="{{ route('admin.warehouse.export.create') }}" method="post" enctype="multipart/form-data">
        @csrf
        <warehouse-export :orders="{{ json_encode($orders) }}"
        quick_code="{{ $code }}" 
        :products="{{ json_encode($products) }}"
        :productions="{{ json_encode($productions->load('product')) }}"></warehouse-export>
    </form>
</div>
