                    ...
                    <tr class="text-gray-700 dark:text-gray-400">
                        ...
                        <td class="px-3 py-3 text-sm flex items-center">
                            ...
                            @elseif($production->status < 4) <!-- <button v-tooltip="'Cập nhật trạng thái'"
                                ...
                                <button title="Báo cáo" v-tooltip="'Báo cáo tiến độ'"
                                    class="flex items-center justify-between px-3 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                                    aria-label="process"
                                    @click="openAnalyzeModal({{ json_encode($production->load(['productions', 'productions.produceds', 'productions.product', 'productions.product.stage_product'])) }})">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20"
                                        fill="currentColor">
                                        <path
...
<transition enter-class="ease-out opacity-0" enter-to-class="opacity-100" leave-class="ease-in opacity-100"
    leave-to-class="opacity-0">
    <div v-show="this.dataAnalyze != null" class="
        fixed
        inset-0
        z-30
        flex
        ...
        bg-black bg-opacity-50
        sm:items-center sm:justify-center
      " id="backdrop-overlay" @click="openAnalyzeModal(null)">
        <transition enter-class="ease-out opacity-0 transform translate-y-1/2" enter-to-class="opacity-100"
            leave-class="ease-in opacity-100" leave-to-class="opacity-0 transform translate-y-1/2">
            <!-- Modal -->
            <div v-show="this.dataAnalyze != null" class="
        ...
        " role="dialog" id="modal" @click.stop="">
              ...
              dark:hover:text-gray-200
              hover: hover:text-gray-700
            " aria-label="close" @click="openAnalyzeModal(null)">
                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20" role="img" aria-hidden="true">
                            <path
                                d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
                                clip-rule="evenodd" fill-rule="evenodd"></path>
                ...
                <div class="mt-4 mb-6">
                    ...
                    <!-- Modal description -->
                    <div v-for="production in this.dataAnalyze?.productions" :key="production.id"
                        ...
                        <div class="col-span-4 bg-gray-300 rounded-md overflow-hidden">
                            <p v-if="calPerProcess(production) <= 0" class="h-8 py-1 text-center">0%</p>
                                <div v-if="calPerProcess(production) > 0" class="h-8 bg-indigo-500 text-center text-white py-1 duration-150 ease-in delay-150" :style="'width:' + calPerProcess(production) + '%'"
                               >[[ (calPerProcess(production)).toFixed(2).replace('.00', '') ]]%</div>                      
                        </div>
                    </div>
