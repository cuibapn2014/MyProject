...
<div class="container px-6 mx-auto grid">
    ...
    @endif
    <form action="{{ route('admin.provider.request.create') }}" method="post">
        @csrf
        <div class="px-4 py-3 mb-8 bg-[#ffffff] rounded-lg shadow-md dark:bg-gray-800">
            ...
            <label class="block text-sm my-2">
              ...
              " name="status" id="status">
                    <option value="1" {{ old('status')==1 ? 'selected' : null }}>Đang hợp tác</option>
                    <option value="2" {{ old('status')==1 ? 'selected' : null }}>Ngưng hợp tác</option>
                </select>
            </label>
            <label class="block mt-4 text-sm">
                ...
                <textarea
                    ...
                    rows="3" placeholder="Nhập ghi chú" name="note" value="{{ old('note') }}"></textarea>
            </label>
