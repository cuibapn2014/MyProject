...
<div class="container px-6 mx-auto grid">
    ...
    @endif
    <form action="{{ route('admin.provider.request.update', ['id' => $provider->id]) }}" method="post">
        @csrf
        <div class="px-4 py-3 mb-8 bg-[#ffffff] rounded-lg shadow-md dark:bg-gray-800">
            <label class="block text-sm">
                ...
                <input
                    ...
                    placeholder="Nhập tên nhà cung cấp" name="name" value="{{ $provider->name }}" />
            </label>
            <label class="block text-sm my-2">
                ...
                <input
                    class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
                    type="text" name="phone_number" placeholder="XXXXXXXXXX" value="{{ $provider->phone_number }}" />
            </label>
            <label class="block text-sm my-2">
                <span class="text-gray-700 dark:text-gray-400">Địa chỉ</span>
                <input
                    class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
                    name="address" placeholder="Nhập địa chỉ" value="{{ $provider->address }}" />
            </label>
            <label class="block text-sm my-2">
              ...
              " name="status" id="status">
                    <option value="1" {{ $provider->status == 'Đang hợp tác' ? 'selected' : null }}>Đang hợp tác</option>
                    <option value="2" {{ $provider->status == 'Ngưng hợp tác' ? 'selected' : null }}>Ngưng hợp tác</option>
                </select>
            </label>
            <label class="block mt-4 text-sm">
                ...
                <textarea
                    ...
                    rows="3" placeholder="Nhập ghi chú" name="note" value="{{ $provider->note }}"></textarea>
            </label>
